    ...
    try:
        ...
        <body>
            ...
            <script>
                ...
                // SIMPLE CHART INITIALIZATION - No conflicts!
                ...
                document.addEventListener('DOMContentLoaded', function() {{
                    ...
                    // Step 2: Initialize chart with our new simple approach
                    setTimeout(() => {{
        ...
        <body>
                ...
                <main class="content">
                    ...
                    <div class="chart-section" style="background: #141414; border: 1px solid #2a2a2a; border-radius: 12px; margin-bottom: 20px; height: 600px; position: relative;">
                        <div class="chart-header" style="padding: 15px; border-bottom: 1px solid #2a2a2a; display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <div style="color: #00d088; font-weight: bold;" id="live-price">LIVE</div>
                        </div>
                        
                        <!-- Multiple Chart Options - Fallback System -->
                        <div style="height: calc(100% - 60px); width: 100%; position: relative;">
                            
                            <!-- Primary: TradingView Advanced Chart -->
                            ...
                            <div id="fallback-chart" style="display: none; height: 100%; background: #1a1a1a; border-radius: 0 0 12px 12px;">
                                <div style="height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 40px;">
                                    <div style="font-size: 64px; color: #00d4aa; margin-bottom: 20px;">📈</div>
                                    <h2 style="color: #00d4aa; margin-bottom: 20px;">Live Gold Price</h2>
                                    <div id="fallback-price" style="font-size: 48px; font-weight: bold; color: #ffd700; margin-bottom: 15px;">
                                        $2000.00
                                    </div>
                                    <div id="fallback-change" style="color: #00d084; font-size: 18px; margin-bottom: 30px;">
                                        +5.50 (+0.27%)
                                    </div>
                                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 30px; font-size: 14px;">
                                        <div style="text-align: center;">
                                            <div style="color: #888;">24h High</div>
                                            <div style="color: #fff; font-weight: bold;">$2010.50</div>
                                        </div>
                                        <div style="text-align: center;">
                                            <div style="color: #888;">24h Low</div>
                                            <div style="color: #fff; font-weight: bold;">$1995.20</div>
                                        </div>
                                        <div style="text-align: center;">
                                            <div style="color: #888;">Volume</div>
                                            <div style="color: #fff; font-weight: bold;">1.2M oz</div>
                                        </div>
                                    </div>
                                    <a href="https://www.tradingview.com/chart/?symbol=OANDA:XAUUSD" target="_blank" 
                                       style="background: #00d4aa; color: white; padding: 15px 30px; border-radius: 8px; text-decoration: none; font-weight: bold; display: inline-block;">
                    ...
                    <script>
                        // BULLETPROOF Chart protection system
                        let chartProtected = false;
                        
                        function protectChart() {{
                            const chartSection = document.querySelector('.chart-section');
                            const tvWidget = document.getElementById('tradingview-widget');
                            
                            if (chartSection && !chartProtected) {{
                                console.log('🛡️ PROTECTING CHART from being overwritten...');
                                
                                // Lock the chart section completely
                                Object.defineProperty(chartSection, 'innerHTML', {{
                                    set: function(value) {{
                                        console.log('🚫 BLOCKED attempt to replace chart content!');
                                        return false;
                                    }},
                                    get: function() {{
                                ...
                                // Also protect the TradingView widget container
                                if (tvWidget) {{
                                    Object.defineProperty(tvWidget, 'innerHTML', {{
                                        set: function(value) {{
                                            console.log('🚫 BLOCKED attempt to replace TradingView widget!');
                                            return false;
                                        }},
                                        get: function() {{
                                            return this.childNodes;
                                ...
                                chartProtected = true;
                                console.log('✅ Chart is now PROTECTED from modifications');
                            }}
                        }}

                        // Chart fallback system with protection
                        function initializeChart() {{
                            const tvWidget = document.getElementById('tradingview-widget');
                            const fallbackChart = document.getElementById('fallback-chart');
                            
                            // IMMEDIATELY protect the chart
                            protectChart();
                            
                            setTimeout(function() {{
                                // Check if TradingView widget loaded
                                const hasIframe = tvWidget && tvWidget.querySelector('iframe');
                                
                                if (!hasIframe) {{
                                    console.log('📊 TradingView widget not detected, showing fallback chart...');
                                    if (tvWidget) tvWidget.style.display = 'none';
                                    if (fallbackChart) fallbackChart.style.display = 'block';
                                    
                                    // Update fallback with real data
                                    updateFallbackPrice();
                                    setInterval(updateFallbackPrice, 5000);
                                }} else {{
                                    console.log('✅ TradingView widget loaded successfully!');
                                }}
                                
                                // Re-protect after any potential modifications
                                protectChart();
                            }}, 3000);
                        }}
                        ...
                        function updateFallbackPrice() {{
                            fetch('/api/price')
                                ...
                                .then(data => {{
                                    ...
                                    const liveElement = document.getElementById('live-price');
                                    
                                    if (priceElement) priceElement.textContent = `$${data.price}`;
                                    if (changeElement) {{
                                        const changeColor = data.change >= 0 ? '#00d084' : '#ff4757';
                                    ...
                                    if (liveElement) liveElement.textContent = `$${data.price}`;
                                }})
                        ...
                        // AGGRESSIVE protection - monitor for chart destruction every 500ms
                        setInterval(function() {{
                            const chartSection = document.querySelector('.chart-section');
                            if (chartSection && chartSection.style.display === 'none') {{
                                console.log('🚨 CHART HIDDEN! Forcing it back...');
                                chartSection.style.display = 'block';
                                protectChart();
                            }}
                            
                            const tvWidget = document.getElementById('tradingview-widget');
                            if (tvWidget && (!tvWidget.querySelector('.tradingview-widget-container') && !tvWidget.querySelector('iframe'))) {{
                                console.log('🚨 TRADINGVIEW WIDGET DESTROYED! Showing fallback...');
                                const fallbackChart = document.getElementById('fallback-chart');
                                if (fallbackChart) fallbackChart.style.display = 'block';
                            }}
                        }}, 500);
            ...
            <script>
                ...
                window.addEventListener('load', () => {{
                    ...
                    // OLD: Load TradingView chart with enhanced protection
                    // loadTradingViewChart(); // DISABLED - using direct HTML embed
...
def get_ml_predictions():
    ...
    try:
        ...
        predictions = {
            ...
            'target_price': round(random.uniform(1980, 2020), 2),
            'stop_loss': round(random.uniform(1950, 1980), 2),
    ...
    try:
        ...
        analysis = {
            ...
            'technical': {
                ...
                'bollinger': random.choice(['OVERBOUGHT', 'OVERSOLD', 'NEUTRAL']),
                'support': round(random.uniform(1970, 1990), 2),
                'resistance': round(random.uniform(2010, 2030), 2)
            },
            'news_impact': {
                'events': random.randint(3, 10),
                'impact_score': random.uniform(0.3, 0.8),
                'key_topics': ['Fed Policy', 'Inflation', 'Dollar Strength']
            }
        }
...
def get_portfolio():
    ...
    try:
        ...
        portfolio = {
            'balance': round(random.uniform(8000, 12000), 2),
            'equity': round(random.uniform(9000, 11000), 2),
            'profit_loss': round(random.uniform(-500, 1000), 2),
            'profit_loss_percent': round(random.uniform(-5, 10), 2),
            'positions': [
                {
                    'symbol': 'XAUUSD',
                    'side': random.choice(['BUY', 'SELL']),
                    'size': random.uniform(0.1, 2.0),
                    'entry_price': round(random.uniform(1990, 2010), 2),
                    'current_price': round(random.uniform(1995, 2005), 2),
